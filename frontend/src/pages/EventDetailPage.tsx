...
export default function EventDetailPage() {
  ...
  const isCreator =
    user && event && Number(user.id) === Number(event.creator_id);

  useEffect(() => {
    if (!eventId || !isHydrated) return;
    getRsvps(eventId)
      .then((res) => {
        setRsvpSummary(res.data.summary);
        setMyRsvp(res.data.user_rsvp?.status ?? null);
      })
      .catch(() => {});
  }, [eventId, isHydrated]);
  ...
  const handleRsvp = async (status: RsvpStatus) => {
    ...
    try {
      ...
      if (myRsvp === status) {
        ...
        const res = await upsertRsvp(eventId, status);
        setMyRsvp(res.data.rsvp.status);
      }
      const updated = await getRsvps(eventId);
      setRsvpSummary(updated.data.summary);
    } catch {
      setRsvpError("Couldn't update RSVP.");
    } finally {
      setRsvpBusy(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate("/events/mine", { replace: true });
    } catch {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {deleteError && (
          <div className="mb-5 rounded-xl border border-danger/25 bg-danger-bg px-4 py-3 text-sm text-danger">
            {deleteError}
          </div>
        )}
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_272px]">
          <div className="space-y-4">
            ...
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {isPast && (
                  <span className="rounded-xl border border-border bg-surface px-2.5 py-1 text-xs text-muted">
                    Past event
                  </span>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              {event.description ? (
                <p className="whitespace-pre-wrap text-[15px] leading-7 text-foreground-secondary">
                  {event.description}
                </p>
              ) : (
                <p className="text-sm italic text-placeholder">
                  No description.
                </p>
              )}
              {event.tags?.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {event.tags.map((t) => (
                    <span
                      key={t.id}
                      className="rounded-xl bg-primary/8 px-3 py-1 text-xs font-medium text-primary"
                    >
                      {t.name}
                    </span>
                  ))}
            ...
            {!isPast && (
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                {rsvpError && (
                  <p className="mb-3 text-sm text-danger">{rsvpError}</p>
                )}
                <div className="flex flex-wrap gap-2">
                  {(
                    ...
                    <button
                      ...
                      className={[
                        "flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-colors disabled:opacity-40",
                        myRsvp === status
          ...
          <aside className="space-y-3">
            <div className="rounded-2xl border border-border bg-card p-5">
              <p className="mb-4 text-xs font-semibold uppercase tracking-wide text-placeholder">
                Event details
              </p>
              <div className="space-y-3.5">
                ...
                <Detail icon={<Users size={14} />} label="Organiser">
                  {event.creator_name}
                </Detail>
              </div>
            </div>
            ...
            {rsvpSummary && (
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <div className="space-y-3">
                  <Bar
                    label="Going"
                    count={rsvpSummary.yes}
                    total={
                      rsvpSummary.yes + rsvpSummary.maybe + rsvpSummary.no || 1
                    }
                    color="bg-primary"
                  />
                  <Bar
                    label="Maybe"
                    count={rsvpSummary.maybe}
                    total={
                      rsvpSummary.yes + rsvpSummary.maybe + rsvpSummary.no || 1
                    }
                    color="bg-warning"
